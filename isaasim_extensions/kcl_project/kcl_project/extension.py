...
class KclProjectExtension(omni.ext.IExt):
    def on_startup(self, ext_id):
        ...
        # Set joint control callback
        self._widget.set_joint_control_callback(self._on_joint_control_command)
    
    def _on_joint_control_command(self, command_type, command_data):
        """Handles the joint control command from the UI."""
        carb.log_info(f"[KCL Project] Received joint control command: {command_type}, data: {command_data}")
        
        if command_type == 'preset_angles':
            self._apply_preset_joint_angles(command_data)
        elif command_type == 'method_changed':
            self._current_joint_control_method = command_data
    ...
    def _apply_preset_joint_angles(self, joint_angles):
        """Apply preset joint angles to Canadarm2."""
        if not hasattr(self, '_visual_inspection_canadarm2_path') or not self._visual_inspection_canadarm2_path:
            carb.log_error("[KCL Project] Canadarm2 not loaded for Visual Inspection Mode")
            return
        
        if len(joint_angles) != 7:
            carb.log_error(f"[KCL Project] Expected 7 joint angles, got {len(joint_angles)}")
            return
        ...
        try:
            ...
            # Joint names corresponding to joint_1 through joint_7
            joint_names = ["joint_1", "joint_2", "joint_3", "joint_4", "joint_5", "joint_6", "joint_7"]
            
            carb.log_info(f"[KCL Project] Applying preset joint angles: {joint_angles}")
            
            # Apply angles to each joint
            for i, angle_degrees in enumerate(joint_angles):
                joint_name = joint_names[i]
                joint_path = f"{self._visual_inspection_canadarm2_path}/joints/{joint_name}"
                joint_prim = stage.GetPrimAtPath(joint_path)
                
                if joint_prim and joint_prim.IsValid():
                    # Clamp angle to joint limits (-180 to 180 degrees)
                    angle_clamped = max(-180.0, min(180.0, angle_degrees))
                    
                    # Get the joint and set position
                    joint = UsdPhysics.RevoluteJoint(joint_prim)
                    if joint:
                        # Use physics drive to move to position
                        drive = UsdPhysics.DriveAPI.Apply(joint_prim, "angular")
                        if drive:
                            drive.GetTargetPositionAttr().Set(angle_clamped)
                            
                            carb.log_info(f"[KCL Project] Set {joint_name} to {angle_clamped}°")
                        else:
                            carb.log_warn(f"[KCL Project] Could not create drive for joint {joint_name}")
                    else:
                        carb.log_warn(f"[KCL Project] Joint {joint_name} is not a revolute joint")
                else:
                    carb.log_warn(f"[KCL Project] Joint prim not found: {joint_path}")
            
            carb.log_info("[KCL Project] Preset joint angles applied successfully")
            
        except Exception as e:
            carb.log_error(f"[KCL Project] Error applying preset joint angles: {str(e)}")
    
    def _direct_joint_control(self, details):
        """Handle direct joint control commands."""
        if not hasattr(self, '_visual_inspection_canadarm2_path') or not self._visual_inspection_canadarm2_path:
            carb.log_error("[KCL Project] Canadarm2 not loaded for Visual Inspection Mode")
            return
            
        carb.log_info(f"[KCL Project] Direct joint control: {details}")
        
        # Handle different direct control commands
        if details == 'home_position':
            self._move_joints_to_home()
        elif details == 'inspect_position':
            self._move_joints_to_inspect()
        elif details == 'extend_arm':
            self._move_joints_extend_arm()
        elif details == 'rotate_wrist':
            self._move_joints_rotate_wrist()
        else:
            carb.log_warn(f"[KCL Project] Unknown direct control command: {details}")
    
    def _ros2_joint_control(self, details):
        """Handle ROS2 joint control commands."""
        carb.log_info(f"[KCL Project] ROS2 joint control: {details}")
        
        if details == 'launch_joint_state_publisher':
            self._launch_ros2_joint_state_publisher()
        elif details == 'connect_ros2_control':
            self._connect_ros2_control_interface()
        elif details == 'launch_rviz':
            self._launch_rviz_for_joint_control()
        else:
    ...
    def _move_joints_to_home(self):
        """Move Canadarm2 to home position using direct joint control."""
        try:
            import omni.usd
            from pxr import UsdPhysics
            ...
            self._apply_preset_joint_angles(home_positions)
            carb.log_info("[KCL Project] Moving Canadarm2 to home position")
            
        except Exception as e:
            carb.log_error(f"[KCL Project] Error moving to home position: {str(e)}")
    
    def _move_joints_to_inspect(self):
        """Move Canadarm2 to visual inspection position."""
        try:
            # Inspection position - position end effector to view DragonX
            inspect_positions = {
                "joint_1": -45.0,
                "joint_2": 30.0,
                "joint_3": 15.0,
            ...
            self._apply_joint_positions(inspect_positions)
            carb.log_info("[KCL Project] Moving Canadarm2 to inspection position")
            
        except Exception as e:
            carb.log_error(f"[KCL Project] Error moving to inspection position: {str(e)}")
    
    def _move_joints_extend_arm(self):
        ...
        try:
            ...
            self._apply_joint_positions(extend_positions)
            carb.log_info("[KCL Project] Extending Canadarm2")
            
        except Exception as e:
            carb.log_error(f"[KCL Project] Error extending arm: {str(e)}")
    
    def _move_joints_rotate_wrist(self):
        """Rotate wrist for better viewing angle."""
        try:
            # Current positions with wrist rotation
            rotate_positions = {
                "joint_1": -45.0,
                "joint_2": 30.0,
                "joint_3": 15.0,
                "joint_4": -60.0,
                "joint_5": 45.0,
                "joint_6": 90.0,  # Rotate wrist
                "joint_7": 45.0   # Add roll for better angle
            }
            
            self._apply_joint_positions(rotate_positions)
            carb.log_info("[KCL Project] Rotating Canadarm2 wrist")
            
        except Exception as e:
            carb.log_error(f"[KCL Project] Error rotating wrist: {str(e)}")
    
    def _apply_joint_positions(self, joint_positions):
        """Apply joint positions to the Canadarm2 robot."""
        try:
            import omni.usd
            from pxr import UsdPhysics
            import math
            ...
            # Joint name mapping to USD paths
            joint_mapping = {
                "joint_1": f"{self._visual_inspection_canadarm2_path}/joints/joint_1",
                "joint_2": f"{self._visual_inspection_canadarm2_path}/joints/joint_2",
                "joint_3": f"{self._visual_inspection_canadarm2_path}/joints/joint_3",
                "joint_4": f"{self._visual_inspection_canadarm2_path}/joints/joint_4",
                "joint_5": f"{self._visual_inspection_canadarm2_path}/joints/joint_5",
                "joint_6": f"{self._visual_inspection_canadarm2_path}/joints/joint_6",
                "joint_7": f"{self._visual_inspection_canadarm2_path}/joints/joint_7"
            }
            
            # Apply positions to each joint
            for joint_name, angle_degrees in joint_positions.items():
                if joint_name in joint_mapping:
                    joint_path = joint_mapping[joint_name]
                    joint_prim = stage.GetPrimAtPath(joint_path)
                    ...
                    if joint_prim and joint_prim.IsValid():
                        ...
                        if joint:
                            ...
                            joint.GetAxisAttr().Set("X")  # Ensure axis is set
                    ...
                    carb.log_warn(f"[KCL Project] Unknown joint name: {joint_name}")
    ...
    def setup_lidar_visualization(self, sensor_id):
        ...
        if not hasattr(self, '_rtx_lidars') or not self._rtx_lidars:
            carb.log_info("[KCL Project] LiDAR sensors not yet initialized, skipping visualization setup")
            return False
            ...
            if hasattr(self, '_docking_imus'):
                try:
                    ...
                    for imu_name in self._docking_imus.keys():
                        carb.log_info(f"[KCL Project] Refreshing {imu_name}")
    ...
    def _on_load_visual_inspection_scene(self):
        ...
        carb.log_info("[KCL Project] Starting Visual Inspection Mode loading...")
        ...
        carb.log_info(f"[KCL Project] Loading DragonX from: {dragon_usdz}")
        spawn_usdz_asset(
            stage,
            dragon_usdz,
            prim_path="/World/DragonX",
            position=(11.8, -77.4, 90.6),  # Specific position for Visual Inspection
            scale=(0.1, 0.1, 0.1),
            rotation=(90, 0, 0)  # Oriented for visual inspection
        )
        carb.log_info("[KCL Project] DragonX loaded successfully")

        # 4. Spawn ISS (visual only, no physics)
        iss_usdz = os.path.abspath(os.path.join(ext_path, "data", "prims", "ISS.usdz"))
        carb.log_info(f"[KCL Project] Loading ISS from: {iss_usdz}")
        spawn_usdz_asset(
            stage,
            iss_usdz,
            prim_path="/World/ISS",
        ...
        carb.log_info("[KCL Project] ISS loaded successfully")

        # 5. Create cameras for visual inspection
        carb.log_info("[KCL Project] Creating cameras...")
        ...
        carb.log_info("[KCL Project] All cameras created successfully")

        # 6. Load Canadarm2 with specific positioning for Visual Inspection Mode (as child of ISS)
        carb.log_info("[KCL Project] Starting Canadarm2 loading process...")
        canadarm2_usd = os.path.abspath(os.path.join(ext_path, "data", "usd", "CanadaArm2", "CanadaArm2.usd"))
        carb.log_info(f"[KCL Project] Canadarm2 USD path: {canadarm2_usd}")
        carb.log_info(f"[KCL Project] File exists check: {os.path.exists(canadarm2_usd)}")
        
        if os.path.exists(canadarm2_usd):
            carb.log_info("[KCL Project] Canadarm2 USD file found, attempting to load...")
            try:
                ...
                carb.log_info(f"[KCL Project] load_usd_robot_simple result: success={success}, path={canadarm2_path}")
                ...
                if success:
                    ...
                    carb.log_info("[KCL Project] Configuring physics...")
                    configure_space_physics(stage)
                    set_simulation_timestep(60)
                    
                    # Configure robot joints
                    carb.log_info("[KCL Project] Configuring robot joints...")
                    configure_robot_joints_robust(stage, canadarm2_path)
                    
                    # Store Canadarm2 reference for joint control
                    self._visual_inspection_canadarm2_path = canadarm2_path
                    carb.log_info(f"[KCL Project] Stored Canadarm2 path: {self._visual_inspection_canadarm2_path}")
                    
                    # Try to add end effector camera after a small delay to ensure robot structure is loaded
                    def add_end_effector_camera():
                        try:
                            carb.log_info("[KCL Project] Attempting to add end effector camera...")
                            # First, let's find the actual end effector link by checking common naming patterns
                            potential_paths = [
                                f"{canadarm2_path}/link_7",
                                f"{canadarm2_path}/link_71", 
                                f"{canadarm2_path}/end_effector",
                                f"{canadarm2_path}/joint_7_link",
                                f"{canadarm2_path}/ee_link"
                            ]
                            
                            carb.log_info(f"[KCL Project] Checking potential end effector paths: {potential_paths}")
                            
                            end_effector_link = None
                            for path in potential_paths:
                                prim = stage.GetPrimAtPath(path)
                                carb.log_info(f"[KCL Project] Checking path {path}: valid={prim and prim.IsValid()}")
                                if prim and prim.IsValid():
                                    end_effector_link = path
                                    carb.log_info(f"[KCL Project] Found end effector link at: {path}")
                                    break
                            
                            if end_effector_link:
                                # Create camera on the found end effector link
                                camera_path = f"{end_effector_link}/end_effector_camera"
                                carb.log_info(f"[KCL Project] Creating camera at: {camera_path}")
                                create_camera(
                                    stage, 
                                    camera_path, 
                                    (0, 0, -0.076),  # Position relative to end effector
                                    (-90, 0, 0),     # Rotation
                                    focal_length=20.0,
                                    locked=True
                                )
                                carb.log_info(f"[KCL Project] End effector camera created at: {camera_path}")
                            else:
                                # Fallback: create camera directly on the main robot body
                                camera_path = f"{canadarm2_path}/end_effector_camera"
                                carb.log_info(f"[KCL Project] Creating fallback camera at: {camera_path}")
                                create_camera(
                                    stage, 
                                    camera_path, 
                                    (0, 0, -2.0),    # Position at robot base
                                    (-90, 0, 0),     # Rotation
                                    focal_length=20.0,
                                    locked=True
                                )
                                carb.log_info(f"[KCL Project] End effector camera created at robot base: {camera_path}")
                                
                        except Exception as e:
                            carb.log_error(f"[KCL Project] Error creating end effector camera: {str(e)}")
                    
                    # Call the camera creation function
                    add_end_effector_camera()
                    
                    carb.log_info("[KCL Project] Canadarm2 loaded with physics for Visual Inspection Mode")
                else:
                    carb.log_error("[KCL Project] Failed to load Canadarm2 - load_usd_robot_simple returned False")
            except Exception as e:
                carb.log_error(f"[KCL Project] Exception during Canadarm2 loading: {str(e)}")
        else:
            carb.log_error(f"[KCL Project] Canadarm2 USD file not found: {canadarm2_usd}")
        
        carb.log_info("[KCL Project] Visual Inspection Mode loaded successfully - cameras only, no sensors")
